...
namespace LuminousVector
{
	public class UITrackManager : MonoBehaviour, IPointerClickHandler, IDragHandler, IEndDragHandler
	{
		...
		//Remove beat
		public void RemoveBeat(UIBeatManager beat)
		{
			int index = _beats.IndexOf(beat);
			if (index == -1)
				Debug.LogError("Wrong beat");
			track.RemoveBeat(beat.beat);
		...
		public void UpdateBeats()
		{
			foreach (UIBeatManager b in _beats)
			{
				...
				pos.x = (b.time - SongEditor.instance.scrollPos) * SongEditor.instance.timeScale;
				b.positon = pos;
			}
		}
